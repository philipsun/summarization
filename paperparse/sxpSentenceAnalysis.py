...
def ParseOnePaper(fname):
   ...
   # titleitem = soup.findAll(attr={"class":"ltx_title ltx_title_document"})
    ...
    for t in titleitem:
        ...
        section_title_set[sxpsec.id_str]=sxpsec
##    for s in sectionset:
...
##            print para['id'], para.text;
        ...
        if section_title_set.has_key(sxp_para.para_tuple[0]):
            sxpsec = section_title_set[sxp_para.para_tuple[0]]
...
def SentenceParseOnePaper():
    fname = paperpath + '\\' + 'P14-1007.xhtml'
    sxptxt = ParseOnePaper(fname)
##    sxpPPT.MakePPTFromTextA(sxptxt)
    para_textset = []
    for (titlstr,sxpsec) in sxptxt.section_title_set.items():
...
#    para_tfidf = sxpFenciMakeTFIDF.MakeTFIDFForCorpus(para_textset)

    sentenceset = sxptxt.sxpprocess.sentenceset
    sentence_textset = []
    for ss in sentenceset:
        sentc =' '.join( ss.keylist)
        print '****'
        print sentc
        sentence_textset.append(sentc)

#    sentence_tfidf = sxpFenciMakeTFIDF.MakeTFIDFForCorpus(sentence_textset)
