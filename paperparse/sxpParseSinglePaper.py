...
def is_relevantA(sentences, s, t):
    ...
    if max_dis > t:
        return True
    return False
def ProcessSectionPara(fname,inc_mode=0):
    sxptxt = sxpText()
    sxptxt.fname = fname
    ...
    c_c[0,1] = 1
    sec_root = 0
    sec_index = 0 #because there is a root of doc in the section li
    p_index = 0
    title = para_set[1].strip()
    para_list = []
    sec_list =[[0,[u'0',u'0'],title,0]]
    keywords = []
    i = 0
    for eachpara in para_set:
        txtstr = eachpara.strip()
    ...
    for eachsec in sec_list:
        ...
        print i, sxpsec.id, sxpsec.id_str,sxpsec.title
    ...
    for sxp_para in sxp_para_set:
       # c_p[sxp_para.id_sec,sxp_para.id] = 1.0
        ...
        for st in sxpsent_set:
            s = sxpTextEncode.GetUnicode(st.sentence_text)
            if len(context) == 0:
                context.append(st)
...
def ReplaceManySpace(strtxt):
    ...
    return ns
def TestSectionTree():
    pkpath = r'.\paper\single\pk\testdimension_2.txt.pk'
...
##    sxptxt = ProcessSectionPara(fname)
   ...
   # sxptxt.c_c = sxpParseSectionNetwork.ParseSectionNetwork(sxptxt)
    ...
    sxptxt.c_c = sxpParseSectionNetwork.ParseSectionNetworkSinglePaper(sxptxt)
    StoreSxptext(sxptxt, pkpath)

    sxptxt = sxpReadFileMan.LoadSxptext(pkpath)
    n = sxptxt.c_c.shape[0]
    for i in range(n):
        for j in range(n):
            print sxptxt.c_c[i,j]
    StoreSxptext(sxptxt, pkpath)
...
def TestSinglePaper():
    ...
    fname = r'testdimension_2.txt'
    inc_mode =1
    sxptxt = ProcessSectionPara(fname,inc_mode)
    if inc_mode == 0:
        pname = "testdimension_3" +'.pk' #test
    if inc_mode == 1:
        pname = "testdimension_4" +'.pk'
    i = 0
    print '----------------display section'
    for eachsect in sxptxt.section_list:
        print i,', ', eachsect.title,', ',eachsect.id,', ', eachsect.id_str,', ', eachsect.id_set
        i = i + 1
...
def MakeModelFromPk():
    modelpath = r'.\paper\single\model'
    pkpath = r'.\paper\single\pk'
